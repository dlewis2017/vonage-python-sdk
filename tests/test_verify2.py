...
def test_new_request_sms_full():
    ...
    params = {
        ...
        "brand": "ACME, Inc",
        "workflow": [
            {"channel": "sms", "to": "447700900000", "app_hash": "asdfghjklqw"}
        ],
    }
    verify_request = verify2.new_request(params)
...
def test_new_request_error_fraud_check_invalid_account(dummy_data):
    ...
    assert "Your account does not have permission to perform this action." in str(
        err.value
    )
...
def test_new_request_whatsapp_app_hash_error():
    params = {
        "brand": "ACME, Inc",
        "workflow": [
            {"channel": "whatsapp", "to": "447700900000", "app_hash": "asdfqwerzxc"}
        ],
    }
...
def test_new_request_whatsapp_from_field():
    ...
    params = {
        "brand": "ACME, Inc",
        "workflow": [
            {"channel": "whatsapp", "to": "447700900000", "from": "447000000000"}
        ],
    }
    verify_request = verify2.new_request(params)
...
def test_new_request_whatsapp_invalid_sender_error():
    ...
    params = {
        "brand": "ACME, Inc",
        "workflow": [
            {"channel": "whatsapp", "to": "447700900000", "from": "asdfghjkl"}
        ],
    }
...
def test_new_request_whatsapp_sender_unregistered_error():
    ...
    params = {
        "brand": "ACME, Inc",
        "workflow": [
            {"channel": "whatsapp", "to": "447700900000", "from": "447999999999"}
        ],
    }
...
def test_new_request_silent_auth():
    ...
    params = {
        "brand": "ACME, Inc",
        "workflow": [{"channel": "silent_auth", "to": "447700900000"}],
    }
    verify_request = verify2.new_request(params)
...
def test_new_request_error_conflict():
    ...
    with raises(ClientError) as err:
        verify2.new_request(params)
    assert "Concurrent verifications to the same number are not allowed." in str(
        err.value
    )
