from vonage import Client, ClientError, SubaccountsError, AuthenticationError
...
def test_list_subaccounts_error_forbidden():
    ...
    with raises(ClientError) as err:
        client.subaccounts.list_subaccounts()
    assert 'Account 1234adsf is not provisioned to access Subaccount Provisioning API' in str(
        err.value
    )
...
def test_list_subaccounts_error_not_found():
    ...
    with raises(ClientError) as err:
        client.subaccounts.list_subaccounts()
    assert "API key '1234asdf' does not exist, or you do not have access" in str(err.value)
...
def test_transfer_credit_invalid_transfer():
    ...
    assert 'Invalid Transfer' in str(err.value)
...
def test_transfer_credit_insufficient_credit():
    ...
    with raises(ClientError) as err:
        client.subaccounts.transfer_credit(from_='asdfzxcv', to='qwerasdf', amount=1)
    assert 'Insufficient Credit' in str(err.value)
...
def test_transfer_number_invalid_transfer():
    ...
    assert (
        'Could not transfer number 12345678901 from account 1234asdf to asdfzxcv - ShortCode is not owned by from account'
        in str(err.value)
    )
...
def test_transfer_number_error_number_not_found():
    ...
    assert (
        'Could not transfer number 12345678901 from account 1234asdf to asdfzxcv - ShortCode not found'
        in str(err.value)
    )
