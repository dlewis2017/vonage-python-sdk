...
def test_start_verification_error(verify):
    ...
    with pytest.raises(VerifyError) as err:
        ...
        assert (
            str(err.value)
            == 'Verify API method failed with error code 2: Your request is incomplete and missing the mandatory parameter `number`'
        )
...
def test_check_verification_error(verify):
    ...
    with pytest.raises(VerifyError) as err:
        ...
        assert (
            str(err.value)
            == 'Verify.start_verification method failed with error code 16: The code inserted does not match the expected value'
        )
...
def test_search_for_verification_200_error(verify):
    ...
    with pytest.raises(VerifyError) as err:
        ...
        assert (
            str(err.value)
            == 'Verify API method failed with status: IN PROGRESS and error: No response found'
        )


@responses.activate
def test_search_for_verification_error_no_id_provided(verify):
...
@responses.activate
def test_cancel_verification_error(verify):
    stub(
        responses.POST,
        "https://api.nexmo.com/verify/control/json",
    ...
    with pytest.raises(VerifyError) as err:
        ...
        assert (
            str(err.value)
            == "Verify API method failed with status: 6 and error: The requestId 'asdf' does not exist or its no longer active."
        )
...
@responses.activate
def test_start_psd2_verification_error(verify):
    stub(
        responses.POST,
        "https://api.nexmo.com/verify/psd2/json",
        fixture_path='verify/start_verification_error.json',
    ...
    assert (
        str(err.value)
        == 'Verify API method failed with status: 2 and error: Your request is incomplete and missing the mandatory parameter `number`'
    )
...
@responses.activate
def test_start_psd2_verification_blacklisted_error_with_network_and_request_id(client):
    stub(
        responses.POST,
        "https://api.nexmo.com/verify/psd2/json",
        fixture_path="verify/blocked_with_network_and_request_id.json",
