...
def test_send_message(sms, dummy_data):
    stub(responses.POST, "https://rest.nexmo.com/sms/json", fixture_path='sms/send_message.json')
...
def test_send_message_200_error(sms, dummy_data):
    ...
    with pytest.raises(SmsError) as err:
        assert isinstance(sms.send_message(params), dict)
        assert (
            str(err.value) == 'Sms.send_message method failed with error code 2: Missing to param'
        )
...
def test_send_long_message(sms, dummy_data):
    stub(
        responses.POST, "https://rest.nexmo.com/sms/json", fixture_path='sms/send_long_message.json'
    )
    ...
    assert "to=447525856424" in request_body()
    assert (
        f"text={long_message.replace(' ', '+').replace(',', '%2C')}" in request_body()
    )  # Check for encoding
